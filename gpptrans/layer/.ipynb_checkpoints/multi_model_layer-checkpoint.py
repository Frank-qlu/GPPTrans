...
from gpptrans.layer.gatedgcn_layer import GatedGCNLayer
from gpptrans.layer.gine_conv_layer import GINEConvESLapPE
from gpptrans.layer.bigbird_layer import SingleBigBirdLayer
from gpptrans.layer.ETransformer import ETransformer
from gpptrans.layer.GPPTans import GPPTansAttention
# from gpptrans.layer.ops import GraphUnet

from gpptrans.layer.graphunet_with_scales import GraphUnet
    ...
    def forward(self, batch):
        ...
        if self.local_gnn_type == 'CustomGatedGCN':
            ...
            if self.equivstable_pe:
                ...
                # print(self.local_model(h, edge_index))
                h_local = self.local_model(h, edge_index)
            else:
                h_local = self.local_model(h, edge_index, edge_attr)
...
class GlobalModel(nn.Module):
    ...
    def __init__(self, dim_h, global_model_type, edge_type, use_edge_attr, edge_attr_type, num_heads,
        ...
        elif global_model_type == 'GPPTans':
            self.self_attn = GPPTansAttention(dim_h, dim_h, num_heads,
                                          use_bias=False, 
        ...
        self.n_act = getattr(nn, 'ELU')()
        self.g_unet = GraphUnet(
            [0.9,0.8,0.7,0.6,0.5,0.4], dim_h, dim_h, dim_h,self.n_act,
            0.5)

    def forward(self, batch):
        adj = self.edge_index_to_adjacency_matrix(batch.edge_index, batch.x.shape[0]).to(torch.float32)
        h = batch.x
        h_in1 = h  # for first residual connection
        # === 修改处: 启用多尺度可视化 ===
        labels = getattr(batch, 'y', None) #batch.y
        # === 修改处: 启用多尺度可视化 ===
        h = self.g_unet(adj.to(h.device), h,tsne_vis=True, save_dir="./tsne_scales")
        # 后续保持不变 ...
        if self.global_model_type in ['ETransformer', 'GPPTans']:
...
classGPPTansLayer(nn.Module):
    """Any combination of different models can be made here.
      ...
      4. GAT + CustomGatedGCN + GPPTans + Transformer
    """
    ...
    def __init__(self, dim_h,
        ...
        for layer_type in model_types:
            ...
            if layer_type in {'Transformer', 'ETransformer', 'GPPTans', 'Performer', 'BigBird'}:
            ...
            elif layer_type in {'GCN', 'GIN','GENConv', 'GINE', 'GAT', 'PNA', 'CustomGatedGCN'}:
...
class SingleLayer(nn.Module):
    ...
    Difference with the GPPTans is that after each layer there is no combining representations and Feed Forward network.
    """
    ...
    def __init__(self, dim_h,
        ...
        if self.model_type in {'Transformer', 'ETransformer', 'GPPTans', 'Performer', 'BigBird'}:
