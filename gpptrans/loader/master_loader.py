...
from gpptrans.loader.planetoid import Planetoid
from gpptrans.loader.dataset.aqsol_molecules import AQSOL
from gpptrans.loader.dataset.coco_superpixels import COCOSuperpixels
from gpptrans.loader.dataset.malnet_tiny import MalNetTiny
from gpptrans.loader.dataset.voc_superpixels import VOCSuperpixels
from gpptrans.loader.split_generator import (prepare_splits,
                                             set_dataset_splits)
from gpptrans.transform.posenc_stats import compute_posenc_stats
from gpptrans.transform.transforms import (pre_transform_in_memory,
                                           generate_splits, # not the same as split_generator above.
                                           typecast_x, concat_x_and_pos,
                                           clip_graphs_to_size, move_node_feat_to_x)
from gpptrans.transform.expander_edges import generate_random_expander
from gpptrans.transform.dist_transforms import (add_dist_features, add_reverse_edges,
...
def preformat_OGB_Graph(dataset_dir, name):
    ...
    if name == 'ogbg-ppa':
        ...
        from gpptrans.loader.ogbg_code2_utils import idx2vocab, \
...
def preformat_PCQM4Mv2Contact(dataset_dir, name):
    ...
    try:
        # Load locally to avoid RDKit dependency until necessary
        from gpptrans.loader.dataset.pcqm4mv2_contact import \
...
def preformat_Peptides(dataset_dir, name):
    ...
    try:
        # Load locally to avoid RDKit dependency until necessary.
        from gpptrans.loader.dataset.peptides_functional import \
            PeptidesFunctionalDataset
        from gpptrans.loader.dataset.peptides_structural import \
...
def preformat_TUDataset(dataset_dir, name):
    ...
    """
    func = None
    if name in ['DD', 'NCI1','NCI109' 'ENZYMES', 'PROTEINS']:
        func = None
