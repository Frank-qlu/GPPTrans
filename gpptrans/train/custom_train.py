...
from gpptrans.loss.subtoken_prediction_loss import subtoken_cross_entropy
from gpptrans.utils import cfg_to_dict, flatten_dict, make_wandb_name
...
def train_epoch(logger, loader, model, optimizer, scheduler, batch_accumulation):
    ...
    for iter, batch in enumerate(loader):
        ...
        Forward_time.append(int((time.time()-begin1)*1000))
#         print("Model Parameters:... ", count_parameters(model))
        # if len(Forward_time)==25:
        #             print(np.mean(Forward_time[-20:]),np.std(Forward_time[-20:],ddof=1))
        ...
        Backward_time.append(int((end-begin)*1000))
#       print("Backward time:ms",int((end-begin)*1000))
        # if len(Backward_time)==25:
        #     print(np.mean(Backward_time[-20:]),np.std(Backward_time[-20:],ddof=1))
